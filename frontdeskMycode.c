	...
	for(i=1; i<=roomnum; i++)
	{
		...
		if(i == roomnum)
		{
			...
			else
			{
				...
				write(fd,guestname,strlen(guestname));
				return 0;
			}
		}
...
int checkout_guest(char *guestname)
{
	...
	int i;
	char namebuf[NAME_LEN_MAX+1];
	char ncpy[NAME_LEN_MAX+1] = "                    \n";//guestname 공간
	char ex[NAME_LEN_MAX+1] = "                    \n";//체크아웃 빈공간
	strncpy(ncpy,guestname,strlen(guestname));//guestname을ncpy에 복사
	ncpy[NAME_LEN_MAX] = '\0';
	if((fd = open(FILE_NAME, O_RDWR)) == -1)
	{
		printf("fail to open file\n");	exit(0);
	...
	for(i=1; i<=NUM_ROOMS; i++)
	{
		...
		if(strcmp(namebuf,ncpy)==0)// namebuf == guestname (일치)
			{
				lseek(fd,offset,SEEK_SET);
				write(fd,ex,NAME_LEN_MAX);//빈공간만들기
				return i;
			}
	...
	if(result == -1)
	{
		printf("guest %s does not exist in the rooms\n",guestname);
	}
	close(fd);
